pipeline{
    ...
    stages{
        ...
        stage('Start Grid'){
            steps{
                sh "docker-compose -f grid.yaml up --scale ${BROWSER}=2 -d"
            }
        }
        ...
        stage('Run Test'){
            steps{
                ...
                script{
                    if(fileExists('docker-output/flight-reservation/testng-failed.xml') || fileExists('docker-output/flight-reservation/testng-failed.xml')){
